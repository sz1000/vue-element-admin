  ...
  <div class="contant-container" style="margin-top:30px;">
    <div class="earning">
      ...
      >当前订单状态：已支付</div>
       <div
        class="earning-intro"
        style="  width:873px; height:60px;line-height:60px; text-align:left; padding-left:30px; overflow:hidden;"
      >
      <div
        class="left"
        style=" margin-left:-30px; background:rgba(247,247,247,1); width:768px; height:60px;line-height:60px; text-align:left; padding-left:30px; float:left;"
      >
        核销码：{{secret_key}}
         </div>
        <div class="right" style=" background:rgba(24,144,255,1); width:86px; height:60px; color: #ffffff; float:right;  padding-left:30px;border-radius:2px;" @click="open">核销</div>
      ...
      <div class="earning-content">
        ...
        <!-- <div class="earning-intro">
          ...
          <p style="width:300px;">
            <span style="font-weight:500">销核时间：</span>
            <span style="font-weight:500">{{ this.confirm_time }}</span>
          </p> 
         </div>  -->
      </div>
...
import { getListOrdersId,} from "@/api/order";
import { putConfirmOrder } from "@/api/orderForm";
...
export default {
  ...
  methods: {
    async initData() {
      ...
      //核销时间
      this.confirm_time = serveData.data.confirm_time;
      //核销码
      this.secret_key = serveData.data.secret_key;
    ...
    destroy() {
      putConfirmOrder(this.id);
    },
    async open() {
      this.$confirm("是否确认核销本商品？", "提示：请仔细核对商品信息！", {
